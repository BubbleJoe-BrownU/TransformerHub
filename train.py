...
torch.backend.cuda.matmul.allow_tf32 = True # allow tf32 on matmul
torch.backend.cuda.allow_tf32 = True # allow tf32 on cudnn
...
val_data = np.memmap(os.path.join(data_dir, 'val.bin'), dtype=np.uint16, model='r')
...
while True:
    ...
    if grad_clip != 0.0:
        ...
        torch.nn.init.clip_grad_norm_(model.parameters(), grad_clip)
